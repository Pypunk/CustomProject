...
{
	return Raycast(rect.GetVertices(), rayP1, rayP2, hitInfo);
}
...
Point2f utils::GetRandPointInRadius(const Point2f& centerPoint, float radius)
{
	const float xValue{ GetRand(centerPoint.x - radius, centerPoint.x + radius) };
	const float yValue{ GetRand(centerPoint.y - radius,centerPoint.y + radius) };
